...
author: "operations | architects, llc"
output:
...
ccc_LastPeriod <- "20230427"
ccc_ThisPeriod <- "20230808"
leadTimeFile <- "20230127"
...
city_df <- read.dbf("DATA/CENSUS2020TOWNS_POLY.dbf", as.is = TRUE)
...
MSFS_df <- read.csv(paste(path_to_ThisPeriod,'/Sales and Distribution/AU Marijuana Establishment Facility Statistics.csv',sep=''), header=TRUE, sep=",")
MSFS_df<- prepMSFSFile(MSFS_df)

MSFSLastPeriod_df <- read.csv(paste(path_to_LastPeriod,'/Sales and Distribution/AU Marijuana Establishment Facility Statistics.csv',sep=''), header=TRUE, sep=",")
...
MELA_Pending_df<- read.csv(paste(path_to_ThisPeriod,'/Applications and Licensure/AU Marijuana Establishment License and Applications - Approved, Pending, Re-Opened.csv',sep=''), header=TRUE, sep=",")

MELA_P_df <- prepMelaFile(MELA_Pending_df)


MELA_Pending_LastPeriod_df<- read.csv(paste(path_to_LastPeriod,'/Applications and Licensure/AU Marijuana Establishment License and Applications - Approved, Pending, Re-Opened.csv',sep=''), header=TRUE, sep=",")
...
MELA_Approved_df<- read.csv(paste(path_to_ThisPeriod,'/Applications and Licensure/AU Marijuana Establishment License and Applications - Approved.csv',sep=''), header=TRUE, sep=",")
MELA_A_df <- prepMelaFile(MELA_Approved_df)

MELA_Approved_df <- prepMelaApprovedFile(MELA_Approved_df)

MELA_Approved_LastPeriod_df<- read.csv(paste(path_to_LastPeriod,'/Applications and Licensure/AU Marijuana Establishment License and Applications - Approved.csv',sep=''),header=TRUE, sep=",")
...
MPPO_df<- read.csv(paste(path_to_ThisPeriod,'/Sales and Distribution/AU Average Monthly Price per Ounce for Adult-Use Cannabis.csv',sep=''), header=TRUE, sep=",")
...
lT_df<- read_csv(paste(path_to_leadTimeFile,"/Applications and Licensure/AU Marijuana Establishment License and Applications - Approved, Pending, Re-Opened.csv",sep=""), 
    col_types = cols(APPLICATION_CREATED_DATE = col_date(format = "%m/%d/%Y"), 
        CNB_DEEMED_COMPLETE_DATE = col_date(format = "%m/%d/%Y"), 
        APPLICATION_APPROVED_DATE = col_date(format = "%m/%d/%Y"), 
        `CNB_DATE_OF_FINAL_LICENSURE-1` = col_date(format = "%m/%d/%Y"), 
        LIC_START_DATE = col_date(format = "%m/%d/%Y"), 
        LIC_EXPIRATION_DATE = col_date(format = "%m/%d/%Y"), 
        COMMENCE_OPERATIONS_DATE = col_date(format = "%m/%d/%Y")))


colnames(lT_df) <- tolower(colnames(lT_df))
lT_df <- lT_df %>%
  select(license_number, business_name, business_city,license_type,application_status,lic_status,approved_license_type, priority_applicant_type, cultivation_environment, cultivation_tier, application_created_date, cnb_deemed_complete_date,application_approved_date,'cnb_date_of_final_licensure-1',lic_start_date, lic_expiration_date,commence_operations_date)

###############################

### Plant Activity File

plantActivity_df <- read_csv(paste(path_to_ThisPeriod, "/Sales and Distribution/AU Marijuana Establishment Adult-use Plant Activity and Volume.csv", sep=""), col_types = cols(ActivitySummaryDate = col_date(format = "%m/%d/%Y")))
...
h6(paste('Date of baseline files: ', format(as.Date(ccc_ThisPeriod,"%Y%m%d"),format="%b %d, %Y")))
...
This lead time analysis was completed from the set of files dropped on `r format(as.Date(leadTimeFile,"%Y%m%d"),format="%b %d, %Y")` by the Cannabis Control Commission for the Commonwealth of Massachusetts.  <br>
This analysis shows the distribution of lead times for each step in the licensee application process, as well as the total time from application to commencing operations.  
<br>
...
checkboxGroupInput(
  ...
  choices = unique(lT_df$priority_applicant_type),
  selected = unique(lT_df$priority_applicant_type),
  inline = FALSE,
  width = NULL,
  choiceNames = NULL,
  choiceValues = NULL
...
leadTime_df <- reactive({
  ...
  filter(priority_applicant_type %in% input$selectPriorityStatus) %>%
  mutate(A2Ptime = application_approved_date - application_created_date,
         P2FnOtime = `cnb_date_of_final_licensure-1` - application_approved_date,
         FnO2FOtime = commence_operations_date - `cnb_date_of_final_licensure-1`,
...
leadTime_df() %>%
    filter(!is.na(TotalTime))%>%
    ggplot(aes(x=as.numeric(TotalTime), fill=priority_applicant_type, col=I('black'))) +
...
<!--     mutate(Cannabis_Requirements = Quantity * Grams_Flower_Req) %>% -->
